...
internal class RandomSampleCollection
{
    ...
    public static void RunSamples(string[] args)
    {
        var nonce = "1234";
    ...
    private static void RunAuthMethodSamples()
    {
        ...
        var appRoleId = "71fce15a-3edb-4263-9227-9d623f1f9a22";
        var secretId = "df1326a5-07f2-814c-4f83-9a00c99694d6";

        IAuthMethodInfo appRoleAuthMethodInfo = new AppRoleAuthMethodInfo(appRoleId, secretId);

        var authVaultClientSettings = GetVaultClientSettings(appRoleAuthMethodInfo);
    ...
    private static void RunCubbyholeSamples()
    {
        ...
        Assert.ThrowsAsync<VaultApiException>(() =>
            _authenticatedVaultClient.V1.Secrets.Cubbyhole.ReadSecretAsync(secretPath)).Wait();
    ...
    private static void RunTransitSamples()
    {
        ...
        var transitSecretsEngine = new SecretsEngine {Type = SecretsEngineType.Transit, Path = path};
        ...
        _authenticatedVaultClient.V1.Secrets.Transit
            .CreateEncryptionKeyAsync(keyName, new CreateKeyRequestOptions {Exportable = true}, path).Wait();
        ...
        var encryptionResponse = _authenticatedVaultClient.V1.Secrets.Transit
            .EncryptAsync(keyName, encryptOptions, path).Result;
        ...
        */
        ...
        // Generate Data Key
        var dataKeyOptions = new DataKeyRequestOptions {Base64EncodedContext = encodedContext, Nonce = nonce};

        Secret<DataKeyResponse> dataKeyResponse = _authenticatedVaultClient.V1.Secrets.Transit
            .GenerateDataKeyAsync("plaintext", keyName, dataKeyOptions, path).Result;
        ...
        var exportedKey = _authenticatedVaultClient.V1.Secrets.Transit
            .ExportKeyAsync(TransitKeyCategory.encryption_key, keyName, null, path).Result;
        DisplayJson(exportedKey);

        exportedKey = _authenticatedVaultClient.V1.Secrets.Transit
            .ExportKeyAsync(TransitKeyCategory.encryption_key, keyName, "latest", path).Result;
        DisplayJson(exportedKey);
    ...
    private static void RunKeyValueV2Samples()
    {
        ...
        var values = new Dictionary<string, object> {{"k1", "v1"}, {"k2", "v2"}};

        var currentMetadata = _authenticatedVaultClient.V1.Secrets.KeyValue.V2
            .WriteSecretAsync(path, values, mountPoint: kv2SecretsEngine.Path).Result;

        Assert.True(currentMetadata.Data.Version == 1);

        var kv2Secret = _authenticatedVaultClient.V1.Secrets.KeyValue.V2
            .ReadSecretAsync(path, mountPoint: kv2SecretsEngine.Path).Result;
        Assert.True(kv2Secret.Data.Data.Count == 2);

        var paths2 = _authenticatedVaultClient.V1.Secrets.KeyValue.V2.ReadSecretPathsAsync("", kv2SecretsEngine.Path)
            .Result;
        Assert.True(paths2.Data.Keys.Count() == 1);

        var kv2metadata = _authenticatedVaultClient.V1.Secrets.KeyValue.V2
            .ReadSecretMetadataAsync(path, kv2SecretsEngine.Path).Result;
        Assert.True(kv2metadata.Data.CurrentVersion == 1);

        // patch

        Assert.ThrowsAsync<VaultApiException>(() =>
            _authenticatedVaultClient.V1.Secrets.KeyValue.V2.PatchSecretAsync(Guid.NewGuid().ToString(),
                new Dictionary<string, object>(), kv2SecretsEngine.Path)).Wait();

        var newData = new Dictionary<string, object> {{"k2", "newv2"}, {"k3", "v3"}};

        var newCurrentData = _authenticatedVaultClient.V1.Secrets.KeyValue.V2.PatchSecretAsync(path,
            newData, kv2SecretsEngine.Path).Result;

        Assert.True(newCurrentData.Data.Version == 2);

        var kv2SecretNew = _authenticatedVaultClient.V1.Secrets.KeyValue.V2
            .ReadSecretAsync(path, mountPoint: kv2SecretsEngine.Path).Result;
        Assert.True(kv2SecretNew.Data.Data.Count == 3);
        Assert.True((string) kv2SecretNew.Data.Data["k2"] == "newv2");

        _authenticatedVaultClient.V1.Secrets.KeyValue.V2.DestroySecretAsync(path,
            new List<int> {kv2metadata.Data.CurrentVersion}, kv2SecretsEngine.Path).Wait();
    ...
    private static void RunKeyValueV1Samples()
    {
        ...
        var values = new Dictionary<string, object> {{"foo", "kv2"}, {"foo2", 345}};
    ...
    private static void RunSystemBackendSamples()
    {
        ...
        var headException = Assert.ThrowsAsync<VaultApiException>(() =>
            _unauthenticatedVaultClient.V1.System.GetHealthStatusAsync(queryHttpMethod: HttpMethod.Head)).Result;
        ...
        var initOptions = new InitOptions {SecretShares = 10, SecretThreshold = 5};
        ...
        var backendConfig = _authenticatedVaultClient.V1.System
            .GetAuthBackendConfigAsync(authBackends.Data.First().Value.Path).Result;
        ...
        {
            Path = "github1", Type = AuthMethodType.GitHub, Description = "Github auth - test cases"
        };
        ...
        var newBackendConfig = new BackendConfig {DefaultLeaseTtl = 3600, MaximumLeaseTtl = 4200, ForceNoCache = true};
        ...
        // capabilities
        var caps = _authenticatedVaultClient.V1.System.GetTokenCapabilitiesAsync("v1/sys", masterCredentials.RootToken)
            .Result;
        DisplayJson(caps);
        ...
        var headerValue = "X-Forwarded-For";
        var headerValue2 = "X-RequestId";
        ...
        var header = _authenticatedVaultClient.V1.System.GetAuditRequestHeaderAsync(headerValue.ToLowerInvariant())
            .Result;
        DisplayJson(header);
        ...
        */
        ...
        {
            Enabled = true,
            AllowedHeaders = new List<string> {"header1", "header2"},
            AllowedOrigins = new List<string> {"https://origin1", "https://origin2"}
        };
        ...
        */
        ...
        var
            otp = (string) null; // Convert.ToBase64String(Enumerable.Range(0, 24).Select(i => (byte)i).ToArray()); // OTP is wrong length
        ...
        foreach (var masterKey in masterCredentials.MasterKeys)
        {
            rootStatus = _unauthenticatedVaultClient.V1.System
                .ContinueRootTokenGenerationAsync(masterKey, rootStatus.Nonce).Result;
            DisplayJson(rootStatus);

            if (rootStatus.Complete) break;
        }
        ...
        rootStatus = _unauthenticatedVaultClient.V1.System
            .ContinueRootTokenGenerationAsync(masterCredentials.MasterKeys[0], rootStatus.Nonce).Result;
        DisplayJson(rootStatus);
        Assert.True(rootStatus.Started);
        ...
        rootStatus = _unauthenticatedVaultClient.V1.System
            .QuickRootTokenGenerationAsync(masterCredentials.MasterKeys, rootStatus.Nonce).Result;
        DisplayJson(rootStatus);
        Assert.True(rootStatus.Complete);
        Assert.NotNull(rootStatus.EncodedRootToken);
        ...
        */
        ...
        var mountConfig = _authenticatedVaultClient.V1.System
            .GetSecretBackendConfigAsync(secretBackends.Data.First().Key).Result;
        ...
        {
            Type = SecretsEngineType.AWS, Path = "aws1", Description = "e2e tests"
        };
        ...
        _authenticatedVaultClient.V1.System.ConfigureSecretBackendAsync(newSecretBackend.Path,
            new BackendConfig {DefaultLeaseTtl = ttl, MaximumLeaseTtl = ttl, ForceNoCache = true}).Wait();

        var newMountConfig = _authenticatedVaultClient.V1.System.GetSecretBackendConfigAsync(newSecretBackend.Path)
            .Result;
        ...
        */
        ...
        // write a new policy
        var newPolicy = new Policy {Name = "gubdu", Rules = "path \"sys/*\" {  policy = \"deny\" }"};
        ...
        var aclPolicy = _authenticatedVaultClient.V1.System.GetACLPolicyAsync(aclPolicies.Data.Keys.ElementAt(0))
            .Result;
        DisplayJson(aclPolicy);
        Assert.NotNull(aclPolicy);

        // write a new policy
        var newAclPolicy = new ACLPolicy {Name = "gubdu", Policy = "path \"sys/*\" {  policy = \"deny\" }"};
        ...
        var rawValues1 = new Dictionary<string, object> {{"foo", "bar"}, {"foo2", 345}};
        ...
        var rawValues2 = new Dictionary<string, object> {{"foo", "bar2"}, {"foo2", 346}};
        ...
        var newInitOptions = new InitOptions {SecretThreshold = 4, SecretShares = 8};

        rekeyStatus = _unauthenticatedVaultClient.V1.System
            .InitiateRekeyAsync(newInitOptions.SecretShares, newInitOptions.SecretThreshold).Result;
        DisplayJson(rekeyStatus);
        ...
        {
            rekeyProgress = _unauthenticatedVaultClient.V1.System
                .ContinueRekeyAsync(masterCredentials.MasterKeys[j], rekeyNonce).Result;
        ...
        var quick = _unauthenticatedVaultClient.V1.System
            .QuickRekeyAsync(masterCredentials.MasterKeys, rekeyStatus.Nonce).Result;
        DisplayJson(quick);
        ...
        var data = new Dictionary<string, object> {{"key1", "wrappy"}, {"key2", 23}, {"key3", true}};
        ...
        var tokenWrapInfo = _authenticatedVaultClient.V1.System.LookupTokenWrapInfoAsync(wrappedResponse.WrapInfo.Token)
            .Result;
        DisplayJson(tokenWrapInfo);
        Assert.NotEqual(DateTimeOffset.MinValue, tokenWrapInfo.Data.CreationTime);

        var rewrappedResponse = _authenticatedVaultClient.V1.System
            .RewrapWrappedResponseDataAsync(wrappedResponse.WrapInfo.Token).Result;
        ...
        var unwrappedResponse = _authenticatedVaultClient.V1.System
            .UnwrapWrappedResponseDataAsync<Dictionary<string, object>>(rewrappedResponse.WrapInfo.Token).Result;
        ...
        unwrappedResponse = _authenticatedVaultClient.V1.System
            .UnwrapWrappedResponseDataAsync<Dictionary<string, object>>(wrappedResponse.WrapInfo.Token).Result;
        DisplayJson(unwrappedResponse);
    ...
    private static VaultClientSettings GetVaultClientSettings(IAuthMethodInfo authMethodInfo = null)
    {
        var settings = new VaultClientSettings("http://localhost:8200", authMethodInfo)
        {
            AfterApiResponseAction = r =>
            {
                ...
                if (string.IsNullOrWhiteSpace(content)) Console.WriteLine(_responseContent);
            },
            Namespace = "bhjk",
    ...
    public class FooData
    {
        ...
        [JsonProperty("foo2")] public int Foo2 { get; set; }
    }
}